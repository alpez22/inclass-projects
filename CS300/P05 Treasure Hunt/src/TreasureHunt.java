...
//
// Title: Treasure Hunt - TreasureHunt
...
/**
 * This class models the PApplet display window of the cs300 spring 2022 p05 Treasure Hunt adventure style game
 */
public class TreasureHunt extends PApplet {
  // instance data fields - not static
  ...
  public void setup() {
    ...
    this.imageMode(PApplet.CORNERS); // Interprets the x and y position of an image to the position of its upper-left corner on the screen
  ...
  public void initGame() {
    gameObjects = new ArrayList<Clickable>(); // add to objects array list
    RestartGameButton restart = new RestartGameButton(0, 0);
    add(restart);
    ScreenshotButton screenshot = new ScreenshotButton(140, 0);
    add(screenshot);
     ...
     * PApplet reference to backgroundImage instance data field backgroundImage = loadImage("images"
     * + File.separator + "background.png");
     * 
     * // TODO // Create a new ArrayList of Clickable and assign its reference to the gameObjects
     * instance // field 
     * gameObjects = new ArrayList<Clickable>();
     * InteractiveObject koala = new InteractiveObject("koala", 350, 65, "What a cute stuffed koala! It looks like a real one!");
     * add(koala);
     * 
     * InteractiveObject coinTarget = new InteractiveObject("coinTarget", 65, 510, "This is where
     * the gold coin must be dropped."); add(coinTarget);
     * 
     * InteractiveObject map = new InteractiveObject("map", 330, 550, "The map says to drop the gold
     * coin there.", coinTarget); add(map);
     */
  }
