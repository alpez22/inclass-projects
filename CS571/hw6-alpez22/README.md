...
RESPONSE:
  (a) Followed: Simple and Intuitive Use & Tolerance of Error
        -Simple and Intuitive Use - is followed because there are simple options for users to choose from and names of these tools aren't complicated. When someone wants to view the chatroom, they can see all of the posts and there is a note that if they want to post they must login. If an individual wants to register, they click on the menu page that says "Register". If an individual wants to login, they click on the menu page that says "LogIn". Once logged in, If an individual wants to log out, they click on the menu page that says "Logout". In the chatrooms now, they pick which one they want to use with the drop down and from there type in their post and see it on the page right away. They then have a large button that allows them to delete it if they want to. These are basic and intuititive options for users that don't require any special knowledge.
        -Tolerance of Error - is followed on the register page when a user is asked to repeat their password. This prevents from a user providing a password that they didn't intend. and if the passwords don't match, they cannot register and will be notified that they don't have matching passwords. This provides a fail safe feature for the users.
  (b) Violated: Equitable use & Tolerance of Error
        -Equitable use - is violated, in one example, with the red button for the delete post. For those that cannot see the color red, deleting their own post won't be as easy to find on the page as others. The page is completely black and white except for the delete button so those that can see red, can find their posts and which they can delete easy, but its not the same for individuals that cannot see red.
        -Tolerance of Error - is violated with the logout button. If an individual accidentally clicks on the logout page which is right next to the chat page, they will be instantly logged out. If we wanted to provide a fail safe feature we should create a popup that double checks with the user that they want to log out before they are logged out. That change would account for the intolerance of error currently on this application.
...
![sketch to improve the interface based on the universal design principles pg1](_figures/questionTen(1).png)

![sketch to improve the interface based on the universal design principles pg2](_figures/questionTen(2).png)
