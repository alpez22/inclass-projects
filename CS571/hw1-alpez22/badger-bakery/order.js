...
function calculateSubtotal() {
	...
	for(let i = 0; i < ITEMS.length; i++){
		let quantityOfI = parseInt(document.getElementById(ITEMS[i] + "-quantity").value);
		let priceOfI = parseFloat(document.getElementById(ITEMS[i] + "-price").innerText);
		sum += quantityOfI*priceOfI;
	}
	return sum;
}

function calculateSalesTax() {
	return getSalesTaxRateForState(document.getElementById("state-tax").value) * calculateSubtotal();
}
