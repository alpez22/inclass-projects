...
-Badger news is more mobile-friendly because of the tools bar can navigate a user between pages and there is more indepth processes that users can do.
-there is also more touch input for users to scroll through pages and update their preferences.
-there are messages when things are loading to let the user know that the website isn't defective.

3. What other gesture(s) could be added to Badger News? What additional functionality would these gesture(s) support?

-pinching the fingers in and out to zoom in and out on the pictures and information of each article
-double tap to like/save an article

4. Is there opportunity to add any microinteraction(s)? If so, what would be the trigger-action pair(s)?

-hold and swipe left to delete or remove an article from a persons current selection of articles
