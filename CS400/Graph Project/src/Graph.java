...
public class Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.distance = extendBy.weight.doubleValue() + copyPath.distance; //new edge weight plus all before
            this.dataSequence = new LinkedList<>();
            for(int i = 0; i < copyPath.dataSequence.size(); i++){
                this.dataSequence.add(copyPath.dataSequence.get(i));
            }
            this.end = extendBy.target;
            this.dataSequence.add(end.data); //new edge is now part of data sequence
        }
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        if(start == null || end == null){
            throw new NoSuchElementException("The data in the starting/ending node is null");
        }
        if(vertices.get(start) == null || vertices.get(end) == null){
            throw new NoSuchElementException("No vertex containing start/end can be found");
        }
        if(start.equals(end)){
            return new Path(vertices.get(end));
        }
        
        PriorityQueue<Path> minPQ = new PriorityQueue<Path>(); 
        Hashtable<NodeType, Boolean> visited = new Hashtable<>();

        Path startingNode = new Path(vertices.get(start)); //path from starting node to starting node
        minPQ.add(startingNode);

        while(!minPQ.isEmpty()){
            //current is the path from start to end
            Path current = minPQ.remove();
            //has this path reached the end node
            if(current.end.equals(vertices.get(end))){
                //current path is the shortest path
                return current;
            }
            // end node has now been visited
            visited.put(current.end.data, true);
            //add all the edges that lead up to the end node to linked list
            for(Edge edge: current.end.edgesLeaving){
                //add path as long as the node hasn't been visited
                if(!visited.containsKey(edge.target.data)){
                    Path currentPath = new Path(current, edge);
                    minPQ.add(currentPath);
                }
            }
        }
    ...
    /**
     * Returns the shortest path between a start and 2 end locations.
     * Uses Dijkstra's shortest path algorithm to find the shortest path out of the given end locations.
     * If both end paths are equal, the path to the first endpoint will be printed
     * 
     * @param start the data item in the starting vertex for the path
     * @param end1 the data item in the first destination vertex for the path
     * @param end2 the data item in the second destination vertex for the path
     * @return list of data item in vertices in order on the shortest path between vertex 
     * with data item start and vertex with data item end, including both start and end 
     * @throws NoSuchElementException when no path from start to end can be found
     *     including when no vertex containing start or end can be found
     */
    public List<NodeType> minimumPath2(NodeType start, NodeType end1, NodeType end2) {
        double cost1 = getPathCost(start, end1);
        double cost2 = getPathCost(start, end2);
        if(cost1 < cost2){
            return dijkstrasShortestPath(start,end1).dataSequence;
        }else if(cost1 > cost2){
            return dijkstrasShortestPath(start,end2).dataSequence;
        }else{
            return dijkstrasShortestPath(start,end1).dataSequence;
        }
    }
    ...
    /**
     * Returns the shortest path between a start and 3 end locations.
     * Uses Dijkstra's shortest path algorithm to find the shortest path out of the given end locations.
     * If end paths are equal, the path to the first endpoint will be printed
     * 
     * @param start the data item in the starting vertex for the path
     * @param end1 the data item in the first destination vertex for the path
     * @param end2 the data item in the second destination vertex for the path
     * @param end3 the data item in the third destination vertex for the path
     * @return list of data item in vertices in order on the shortest path between vertex 
     * with data item start and vertex with data item end, including both start and end 
     * @throws NoSuchElementException when no path from start to end can be found
     *     including when no vertex containing start or end can be found
     */
    public List<NodeType> minimumPath3(NodeType start, NodeType end1, NodeType end2, NodeType end3) {
        double cost1 = getPathCost(start, end1);
        double cost2 = getPathCost(start, end2);
        double cost3 = getPathCost(start, end3);

        //if start -> end1 is shortest path
        if(cost1 < cost2 && cost1 < cost3 || cost1 == cost2 && cost1 < cost3 || 
        cost2 == cost3 && cost1 < cost2 || cost1 == cost3 && cost1 < cost2 || 
        cost1 == cost2 && cost1 == cost3){
            return dijkstrasShortestPath(start,end1).dataSequence;
        }
        //if start -> end2 is shortest path
        else if(cost2 < cost1 && cost2 < cost3 || cost2 == cost3 && cost2 < cost1 || 
        cost1 == cost3 && cost2 < cost1){
            return dijkstrasShortestPath(start,end2).dataSequence;
        }
        //if start -> end3 is shortest path
        else{
            return dijkstrasShortestPath(start,end3).dataSequence;
        }
    }
