...
scheduler(void)
{
  ...
  for(;;){
    ...
    acquire(&ptable.lock);
    int tempNiceLevel = 19;
    int counter = 0;
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		if(p->state != RUNNABLE)
		        continue;
		if(p->niceLevel == tempNiceLevel){
			counter++;
		}
		else if(p->niceLevel < tempNiceLevel){
			tempNiceLevel = p->niceLevel;
			counter = 1;
		}
	}
    
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE || p->niceLevel > tempNiceLevel)
        continue;
