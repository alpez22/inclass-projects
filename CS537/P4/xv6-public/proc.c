...
fork(void)
{
  ...
  *np->tf = *curproc->tf;
	...
	//cprintf("\n\n\nSTARTING ADDR: %x\n\n\n", np->allAddresses[0].startingVirtualAddr);
  ...
  for(int i=0; i<16; i++){
  	for(uint j=0; j<16; j++){
  		...
  		/*if(curproc->allAddresses[i].isPrivate == 0){
  			char* arr = (char*)(curproc->allAddresses[i].startingVirtualAddr);
  			char* arr2 = (char*)(np->allAddresses[i].startingVirtualAddr);
  			cprintf("CHAR: %d %d\n", arr2[0], arr[0]);
  			for(uint a=0; a<PGSIZE*curproc->allAddresses[a].physicalPageNumber; a++){
  				arr2[a] = arr[a];
  			}
  		}*/
  	
  		/*uint addr = curproc->allAddresses[i].startingVirtualAddr + (j * PGSIZE);
  		if(addr==0x60001000 )
  			cprintf("ADDRESS: %x\n", addr);
		...
		if(addr==0x60001000 )
		  			cprintf("ADDRESS: %x\n", addr);
  		if(walkpgdir(np->pgdir, (void *) addr, 0) == 0){
  			if(addr==0x60001000 )
  			  			cprintf("ADDRESS: %x\n", addr);
  			uint pa = PTE_ADDR(*pte);
  			mappages(np->pgdir, (void*)addr, PGSIZE, pa, PTE_W | PTE_U);
  		}*/
  	}
  }
...
exit(void)
{
  ...
  if(curproc == initproc)
   ...
   if(curproc != initproc){
   	...
   	for(int i=0; i<16; i++){
   	  	for(int a=0; a<16; a++){
   	  		uint startingAddr = curproc->allAddresses[i].startingVirtualAddr;
   	  		...
   	  		//if(curproc->allAddresses[i].startingVirtualAddr != 0)
   	  			//cprintf("ADDR: %x", curproc->allAddresses[i].startingVirtualAddr);
   	  	}
   	  	//cprintf("TOTAL: %d\n", curproc->allAddresses[i].numberOfPages);
   	  }
   }

	for(int i=15; i<=0; i--){
		wunMap(curproc->allAddresses[i].startingVirtualAddr, 1);
	}
